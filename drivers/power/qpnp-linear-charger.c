				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HOT_IBAT_LIMIT:
		if (val->intval) {
			...
			hot_design_current = QPNP_LBC_IBATMAX_MAX;
		}
		break;
#endif
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
...
static int qpnp_chg_load_battery_data(struct qpnp_lbc_chip *chip)
{
	...
	if (node)
	{
		...
		if(chip->use_other_charger)
		{
			batt_jeita_param.cold.i_max = 0;
			batt_jeita_param.cold.v_max = 0;
			batt_jeita_param.cold.t_high = chip->cfg_cold_bat_decidegc;
			batt_jeita_param.cold.t_low = INT_MIN;

			batt_jeita_param.cool.i_max = chip->cfg_cool_bat_chg_ma;
			batt_jeita_param.cool.v_max = chip->cfg_cool_bat_mv;
			batt_jeita_param.cool.t_high = chip->cfg_cool_bat_decidegc;
			batt_jeita_param.cool.t_low = chip->cfg_cold_bat_decidegc;

			batt_jeita_param.normal.i_max = 1500;
			batt_jeita_param.normal.v_max = 4350;
			batt_jeita_param.normal.t_high = chip->cfg_warm_bat_decidegc;
			batt_jeita_param.normal.t_low = chip->cfg_cool_bat_decidegc;

			batt_jeita_param.warm.i_max = chip->cfg_warm_bat_chg_ma;
			batt_jeita_param.warm.v_max = chip->cfg_warm_bat_mv;
			batt_jeita_param.warm.t_high = chip->cfg_hot_bat_decidegc;
			batt_jeita_param.warm.t_low = chip->cfg_warm_bat_decidegc;

			batt_jeita_param.hot.i_max = 0;
			batt_jeita_param.hot.v_max = 0;
			batt_jeita_param.hot.t_high = INT_MAX;
			batt_jeita_param.hot.t_low = chip->cfg_hot_bat_decidegc;
		}
#endif
...
static void check_charging_status_work(struct work_struct *work)
{
	...
	if((((HOT_TEMP_DEFAULT - HOT_TEMP_BUFFER) > batt_temp)&& (bat_present) && (0 == mode)
		...
		|| ((!chip->cfg_charging_disabled) && (bat_present) && (current_max > 2)
