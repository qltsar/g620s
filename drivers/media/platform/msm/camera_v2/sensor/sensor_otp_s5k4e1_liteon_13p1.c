/************************************************************
  ...
  FileName: sensor_otp_s5k4e1_liteon_13p1.c
...
#undef CDBG
#define CONFIG_S5K4E1_CAMERA_DEBUG

#ifdef CONFIG_S5K4E1_CAMERA_DEBUG
...
{
	CDBG("%s,otp_ptr.product_year:0x%x",__func__,otp_ptr.product_year);
	CDBG("%s,otp_ptr.product_month:0x%x",__func__,otp_ptr.product_month);
	CDBG("%s,otp_ptr.product_date:0x%x",__func__,otp_ptr.product_date);
	CDBG("%s,otp_ptr.camera_id:0x%x",__func__,otp_ptr.camera_id);
	CDBG("%s,otp_ptr.supplier_version_id:0x%x",__func__,otp_ptr.supplier_version_id);
	CDBG("%s,otp_ptr.version:0x%x\n",__func__,otp_ptr.version);
	CDBG("%s,otp_ptr.wb_rg:0x%x",__func__,otp_ptr.wb_rg);
	CDBG("%s,otp_ptr.wb_bg:0x%x",__func__,otp_ptr.wb_bg);
	CDBG("%s,otp_ptr.vcm_start:0x%x",__func__,otp_ptr.vcm_start);
	CDBG("%s,otp_ptr.vcm_end:0x%x",__func__,otp_ptr.vcm_end);
	return;
}
/******************************************************************************
...
void s5k4e1_liteon_update_awb_gain(struct msm_sensor_ctrl_t *s_ctrl,
                                             ...
                                             uint32_t b_gain)
{
	CDBG("%s, R_gain:0x%x,G_gain:0x%x,B_gain:0x%x\n",__func__, r_gain, g_gain, b_gain);
	if (r_gain >= 0x100) {
                                          ...
                                          int isMsb)
{
	...
	CDBG("%s reg:0x%x, isMsb:%d ", __func__, reg, isMsb);
	...
	for (layer = S5K4E1_LITEON_LAYER_AWB_START; layer <= S5K4E1_LITEON_LAYER_AWB_END; layer++)
	{
		...
		CDBG("%s reg_values[%d]:0x%x", __func__, i, reg_values[i]);
	}
                                                ...
                                                struct s5k4e1_liteon_otp_struct *current_otp)
{
	...
	{
		CDBG("%s Error! Invalid OTP location:%d \n", __func__, location);
		rc = -1;
		return rc;
	}
...
s5k4e1_liteon_otp_location s5k4e1_liteon_check_awb_otp_location(struct msm_sensor_ctrl_t * s_ctrl)
{
	...
	CDBG("%s read layer 18: reg310d:0x%x, reg310e:0x%x, reg310f:0x%x \n", __func__, reg310d, reg310e, reg310f);
	...
	for (vcm_location = S5K4E1_LITEON_OTP_310D_LSB ; vcm_location <= S5K4E1_LITEON_OTP_310F_LSB; vcm_location++)
	{
		switch (vcm_location)
		{
			...
			case S5K4E1_LITEON_OTP_310F_LSB://reg 0x310f lsb
				...
				break;
			default:
				CDBG("%s Error vcmLocation %d", __func__, vcm_location);
				return;
		}
        CDBG("%s reg=0x%x mask=%d", __func__, reg, mask);
		...
		for (layer = S5K4E1_LITEON_LAYER_VCM_START; layer <= S5K4E1_LITEON_LAYER_VCM_END; layer++)
		{
			...
			CDBG("%s reg_values[%d]:0x%x, reg:%x", __func__, i, reg_values[i], reg);
		}
		...
		{
			CDBG("%s VCM_Otp found, location:%d ", __func__, vcm_location);
            CDBG("%s vcm_start:%d vcm_end:%d", __func__, current_otp->vcm_start, current_otp->vcm_end);
			//found vcm otp
		...
		{
			//error otp
			CDBG("%s Error! Invalide VCM Otp", __func__);
			return;
		}
	}

	CDBG("%s Error! No VCM Otp \n", __func__);
	return;
}
...
static int s5k4e1_liteon_update_otp (struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	{
		CDBG("%s Error!no AWB data in OTP!\n", __func__);
		return -1;
	}
	...
	if ((0 == current_otp.wb_rg) || (0 ==current_otp.wb_bg))
	{
		CDBG("%s Error!Zero RG or BG data! RG:0x%x, BG:0x%x!\n", __func__, current_otp.wb_rg, current_otp.wb_bg);
		return -1;
	}
...
int s5k4e1_liteon_13p1_otp_func(struct msm_sensor_ctrl_t *s_ctrl, int index)
{
	...
	CDBG("%s, rg_ratio_typical=%04x,bg_ratio_typical=%04x\n", __func__,rg_ratio_typical,bg_ratio_typical);
	...
	if (0 == (s5k4e1_liteon_otp_read_flag & S5K4E1_LITEON_OTP_AWB))
	{
		if(0 != s5k4e1_liteon_update_otp(s_ctrl))
		{
			...
			CDBG("%s : No OTP info! return error!\n", __func__);
			return -1;
		}
	}
	else
	{
		CDBG("%s :AWB OTP has already been read out!\n",__func__);
	}
