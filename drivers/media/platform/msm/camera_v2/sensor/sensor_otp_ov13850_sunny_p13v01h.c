...
#undef CDBG
#define CONFIG_OV13850_CAMERA_DEBUG

#ifdef CONFIG_OV13850_CAMERA_DEBUG
...
static bool ov13850_read_otp(struct msm_sensor_ctrl_t *s_ctrl,uint16_t i2c_addr,u16 reg,u8 *buf,u16 count)
{
	...
	for (i=0; i<count; i++) 
	{
		...
		if (ret !=0)
		{
			CDBG("%s fail to read otp with error code %d, i2c_addr=0x%x reg_addr=0x%x\n", __func__,ret,i2c_addr,reg+i);
			return false;
		}
		buf[i] = (val&0xff);
		OTPSUMVAL += buf[i];
...
static bool ov13850_otp_read_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CDBG("module info year 20%02d month %d day %d, SNO. 0x%x  vendor id&version 0x%x\n", buf[0],buf[1],buf[2],buf[3],buf[4]);
	...
	{
		CDBG("%s OTP data is worng for with wrong vender id!!!\n",__func__);
		return false;
	}
}
...
static bool ov13850_otp_read_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	//CDBG("%s OTP data are Rg_high=%x, Rg_low=%x, Bg_high=%x, Bg_low=%x, gbgr_high=%x, gbgr_low=%x!!!\n", __func__, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
...
static void ov13850_otp_set_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CDBG("ov13850_get_otp_from_sensor() ~ %s R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n",__func__, R_gain, B_gain, G_gain);
...
static bool ov13850_otp_read_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CDBG("%s LCS[0]=%x, LSC[244]=%x, LSC[245]=%x, LSC[359]=%x\n",__func__,
...
static void ov13850_otp_set_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CDBG("%s, set OTP LSC to sensor OK.\n", __func__);
...
static void ov13850_otp_get_vcm(struct msm_sensor_ctrl_t *s_ctrl,u16 *vcm_start, u16 *vcm_end)
{
	...
	CDBG("%s, vcm_start:%#x, vcm_end:%#x", __func__, *vcm_start, *vcm_end);
}
#endif
...
static bool ov13850_get_otp_from_sensor(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	{
		CDBG("%s OTP data is worng, ov13850_otp_flag=0x%04x\n", __func__,otpflag);
		return false;
	}
	else if((otpflag & OV13850_OTP_CHECKSUM_READ) == OV13850_OTP_CHECKSUM_READ)
	{ 
		CDBG("%s OTP has been read success already, ov13850_otp_flag=0x%01x\n", __func__,otpflag);
		return true;
	}
	...
	{
		CDBG("%s OTP data has not flashed!\n", __func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal)
	{
		CDBG("%s ov13850_otp_read_id() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal)
	{
		CDBG("%s ov13850_otp_read_awb() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal)
	{
		CDBG("%s ov13850_otp_read_vcm() failed!\n",__func__);
		goto OTP_FAIL;
	}
...
int ov13850_sunny_p13v01h_otp_func(struct msm_sensor_ctrl_t * s_ctrl, int index)
{
	...
	CDBG("%s, the OTP read and set end.\n", __func__);
