...
#undef CDBG
#define CONFIG_OV5648_CAMERA_DEBUG

#ifdef CONFIG_OV5648_CAMERA_DEBUG
                                                                   ...
                                                                   struct ov5648_sunny_p5v18g_otp_struct * otp_ptr)
{
    ...
    CDBG("%s,group 2 otp_ptr.product_year:%d\n",__func__,otp_ptr->product_year);
    CDBG("%s,group 2 otp_ptr.product_month:%d\n",__func__,otp_ptr->product_month);
    CDBG("%s,group 2 otp_ptr.product_date:%d\n",__func__,otp_ptr->product_date);
    CDBG("%s,group 2 otp_ptr.camera_id:%d\n",__func__,otp_ptr->camera_id);
    CDBG("%s,group 2 otp_ptr.supplier_version_id:%d\n",__func__,otp_ptr->supplier_version_id);

    ov5648_sunny_p5v18g_otp_clear(s_ctrl);

    if (0 != otp_ptr->product_year)// year is not zero, we assume module info is correct
    {
        CDBG("%s, Module info lays in group 2\n",__func__);
        return 0;
    }
    ...
    if (0 != otp_ptr->product_year) // year is not zero, we assume module info is correct
    {
        CDBG("%s, Module info lays in group 1\n",__func__);
        return 0;//otp read sucess
    }
    else
    {
        CDBG("%s,  Error !No Module inf found in OTP\n",__func__);
                                                   ...
                                                   struct ov5648_sunny_p5v18g_otp_struct * otp_ptr)
{
    ...
    {
        CDBG("%s, AWB info lays in group 1\n",__func__);
        return 0;//otp read sucess
    }
    else
    {
        CDBG("%s,  Error! No AWB info found in OTP\n",__func__);
        return -1;//otp read fail
    }
    return 0;
}
                                                         ...
                                                         struct ov5648_sunny_p5v18g_otp_struct * otp_ptr)
{
	...
	{
		CDBG("%s,  Error VCM info is not correct!\n",__func__);
                                             ...
                                             struct ov5648_sunny_p5v18g_otp_struct * current_otp)
{
    ...
    CDBG("%s, ov5648_sunny_p5v18g_otp_read_flag:0x%x\n",__func__, ov5648_sunny_p5v18g_otp_read_flag);
    return;
}
...
static int ov5648_sunny_p5v18g_update_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    {
        CDBG("%s, Error Zero rg or bg, rg:%d, bg:%d\n", __func__, rg, bg);
        return -1;
    }

    CDBG("%s, r/g:0x%x, b/g:0x%x\n", __func__, rg, bg);
