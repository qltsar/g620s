...
{
    int rc;
    U8 before_addr = i2c_client->addr;
    ...
    if(size>8&&NULL!=I2CDMABuf_va)
    {
        ...
        rc = i2c_master_recv(i2c_client, (unsigned char *)I2CDMABuf_pa, size);
        for(i = 0; i < size; i++)
   		{
...
static int ito_test_i2c_write(u8 addr, u8* data, u16 size)//modify : ?¨´?Y????DT?? i2c_client
{
    ...
    if(size>8&&NULL!=I2CDMABuf_va)
	{
		...
		i2c_client->ext_flag = i2c_client->ext_flag | I2C_DMA_FLAG ;
		rc = i2c_master_send(i2c_client, (unsigned char *)I2CDMABuf_pa, size);
	}
	else
	{
...
static ITO_TEST_RET ito_test_interface(void)
{
    ...
    _msg_dma_alloc();
#endif
	g_i2c_freq = qup_get_clk_freq(i2c_client->adapter);
    ...
    _msg_dma_free();
#endif
    qup_set_clk_freq(i2c_client->adapter, g_i2c_freq);
	ito_test_reset();
    ito_test_enable_irq();
    ITO_TEST_DEBUG("end\n");
