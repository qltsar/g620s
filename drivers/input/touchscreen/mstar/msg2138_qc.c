...
static const char * MSG21XX_ofilm_update_bin = NULL;
...
void drvDB_WriteReg(u8 bBank, u8 bAddr, u16 bData)
{
	...
	old_i2c_freq = qup_get_clk_freq(i2c_client->adapter);
	qup_set_clk_freq(i2c_client->adapter, I2C_FREQUENCY_50000);
    bWriteData[0]=0x10;
    bWriteData[1] = bBank;
    bWriteData[2] = bAddr;
	...
	qup_set_clk_freq(i2c_client->adapter, old_i2c_freq);
}
...
{
    int i = 0;
    if (id == SWID_OFILM) //ofilm
    {
        for (i = 0; i < 33; i++)
        {
            firmware_data_store(NULL, NULL, &(MSG21XX_ofilm_update_bin[i * 1024]), 1024);
        }
    }
    else
    {
...
void msg21xx_firmware_auto_upgrade(struct work_struct *ft6x06_fw_config_delay) 
{
    ...
    char touch_info[APP_INFO_VALUE_LENTH] = {0};
        ...
        {
             MSG21XX_ofilm_update_bin = Y550_MSG21XX_ofilm_update_bin;
        }
        else if(strncmp(PHONE_NAME_ULC02, msg21xx_data->product_name, sizeof(PHONE_NAME_ULC02)) == 0)
        {
            MSG21XX_ofilm_update_bin = ULC02_MSG21XX_ofilm_update_bin;
        }
        else
        {
