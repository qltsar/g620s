...
{
	return (  /*condition 1: abs(x + y +z) > 3g */
		     ( abs(x + y +z) > excep->x_y_z_max)
			 /* condition 2: x, y, z all near zero, and abs(x)+abs(y)+abs(z) < 0.6g */
			 ||( abs(x)+abs(y)+abs(z) ) < excep->x_y_z_min
			 /*condition 3:  abs(x) > 1.8g*/
...
static void dump_regs_xyz_values(struct work_struct *work)
{
	...
	KIONIX_DBG("%s,raw_x=%d", __func__,acceld->accel_data[acceld->axis_map_x]);
	KIONIX_DBG("%s,raw_y=%d", __func__,acceld->accel_data[acceld->axis_map_y]);
	KIONIX_DBG("%s,raw_z=%d", __func__, acceld->accel_data[acceld->axis_map_z]);
...
*/
static int kx_gs_dump_i2c_exception_status(struct kionix_accel_driver *acceld)
{
 ...
 */
static int kx_gs_check_exception(int x, int y, int z, struct kionix_accel_driver *acceld )
{
	struct gsensor_test_excep *excep = &acceld->gsensor_test_exception;
...
static void gsensor_read_i2c_err_info(struct kionix_accel_driver *acceld)
{
	...
	exception->i2c_scl_val = gpio_get_value(KIONIX_I2C_SCL);
	exception->i2c_sda_val = gpio_get_value(KIONIX_I2C_SDA);
...
static ssize_t gsensor_report_dsm_err(int type,bool auto_report_flag)
{
	...
	switch(type)
	{
		case DSM_GS_I2C_ERROR:
			...
			acceld->gsensor_test_exception.excep_num = DSM_GS_I2C_ERROR;
			used_size = gsensor_dsm_record_i2c_err_info(acceld);
			break;

		case DSM_GS_DATA_ERROR:
			acceld->gsensor_test_exception.excep_num = DSM_GS_DATA_ERROR;
