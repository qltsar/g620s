...
static int kionix_accel_operate(struct kionix_accel_driver *acceld)
{
	...
	KIONIX_DBG("enter operate mode, poll_delay = %d",acceld->poll_delay);
	return 0;
}
...
static void kionix_accel_report_accel_data(struct kionix_accel_driver *acceld)
{
	...
	{
		queue_delayed_work(acceld->accel_workqueue, &acceld->debug_work, DBG_INTERVAL_10S);
						...
						const char *buf, size_t count)
{
	...
	if(kionix_strtok(buf, count, &buf2, delay_count) < 0) {
		...
		KIONIX_DBG("set new acceld->poll_interval=%d",acceld->poll_interval);
