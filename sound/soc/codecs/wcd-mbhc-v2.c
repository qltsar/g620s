				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (mbhc->buttons_pressed) {
			ad_logn("%s: release of button press%d\n",
				 __func__, jack_type);
...
report:
	ad_logn("%s: Valid plug found, plug type %d wrk_cmpt %d btn_intr %d\n",
...
static irqreturn_t wcd_mbhc_hs_ins_rem_irq(int irq, void *data)
{
	...
	if (detection_type) {
		...
		ad_logn("%s: detection_type %d, result2 %d\n", __func__,
...
irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	if (mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET) {
		ad_logn("%s: Plug type is not headset, ignore button press\n",
			 __func__);
		goto done;
	}
	...
	ad_logn("%s: Reporting btn press result1 =%d\n", __func__, result1);
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		if (mbhc->in_swch_irq_handler) {
			ad_logn("%s: Switch irq kicked in, ignore\n",
					__func__);
		} else {
			ad_logn("%s: Reporting btn release\n", __func__);
			wcd_mbhc_jack_report(mbhc, &mbhc->button_jack,
